...
const db = {
  ...
  all2(sql, params = []) {
    ...
    return Promise.resolve(res[0].values.map(v => {
...
async function seedData() {
  ...
  ]) { const r = await db.run2('INSERT INTO branches (name,address,manager,phone) VALUES (?,?,?,?)', [name,addr,mgr,ph]); bIds.push(r.lastID); }

  const pIds = [];
  for (const [bi,name,cat,unit,du,stock] of [
    [0,"Un (Bug'doy)",'Oziq-ovqat','kg',5,8], [0,'Guruch','Oziq-ovqat','kg',3,25],
    [0,'Benzin',"Yoqilg'i",'litr',20,15],     [0,"Qog'oz (A4)",'Ofis','quti',2,12],
    [0,'Chiroq (LED)','Elektr','dona',0.5,1],
    [1,'Sabzi','Oziq-ovqat','kg',4,6],         [1,'Piyoz','Oziq-ovqat','kg',3,20],
    [1,'Dizel yoqilgisi',"Yoqilg'i",'litr',15,5], [1,'Printer kartrij','Ofis','dona',0.5,3],
    [1,'Elektr simlar','Elektr','rulon',0.3,10],
    [2,'Kartoshka','Oziq-ovqat','kg',6,10],    [2,"O'simlik yog'i",'Oziq-ovqat','litr',2,14],
    [2,'Gaz ballon',"Yoqilg'i",'dona',1,0],    [2,'Ruchkalar (quti)','Ofis','quti',1,8],
    [2,'Rozetka (komplekt)','Elektr','dona',0.2,5],
    [3,'Makaron','Oziq-ovqat','kg',4,3],        [3,'Tuz','Oziq-ovqat','kg',1,12],
    [3,'Moy (motor)',"Yoqilg'i",'litr',0.5,1], [3,"Uy jihozlari (to'plam)","Uy-ro'zg'or",'dona',0.1,8],
    [3,'Skotch lenta','Ofis','dona',2,20],
  ]) { const r = await db.run2('INSERT INTO products (branch_id,name,category,unit,daily_usage,current_stock) VALUES (?,?,?,?,?,?)', [bIds[bi],name,cat,unit,du,stock]); pIds.push(r.lastID); }

  function ago(n) { const d=new Date(); d.setDate(d.getDate()-n); return d.toISOString().split('T')[0]; }
  for (const [pi,qty,price,n,sup] of [
  ...
  ]) await db.run2('INSERT INTO purchases (product_id,quantity,unit_price,purchase_date,supplier) VALUES (?,?,?,?,?)', [pIds[pi],qty,price,ago(n),sup]);
