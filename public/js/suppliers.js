  ...
  c.innerHTML = `
    ...
    <div class="card" style="margin-bottom:16px;padding:14px 18px;background:linear-gradient(135deg,#f0fdf4,#fff);border-color:#86efac">
      ...
      <div style="font-size:13px;color:#15803d;line-height:1.7">
        1. Ta'minotchi qo'shing va uning Telegram ga <b>Ulash havolasi</b> yuboring<br>
...
function renderSuppliersList(el) {
  if (!suppliers.length) {
    el.innerHTML = `<div class="empty-state"><i class="ti ti-users-off"></i><p>Ta'minotchi yo'q. Birinchi ta'minotchini qo'shing.</p></div>`;
    return;
  }
  el.innerHTML = `<div class="table-wrap"><table>
    ...
    ${suppliers.map(s => `<tr>
      ...
      <td>${s.telegram_username ? `<a href="https://t.me/${s.telegram_username.replace('@','')}" target="_blank" style="color:var(--teal)">@${esc(s.telegram_username.replace('@',''))}</a>` : '<span style="color:#94a3b8">—</span>'}
  ...
  openModal(`
    ...
    <div class="modal-body">
      ...
      <div style="background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:9px 13px;font-size:12px;color:#92400e;margin-bottom:4px">
        ⏳ Saqlagandan keyin <b>"Ulash havolasi"</b> ni nusxalab ta'minotchiga yuboring — u bot ga /start bosadi va ulanadi.
      </div>`}
